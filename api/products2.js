...
export default async function handler(req, res) {
  try {
    ...
    const fields = ["id", "name", "list_price", "categ_id"]; // ← NADA de __last_update
    ...
    const items = recs.map(r => ({
      ...
      image: `${ODOO_URL}/web/image?model=product.template&id=${r.id}&field=image_1920&width=${WIDTH}&height=${HEIGHT}`,
    }));

    res.setHeader("X-Api-Version", "products2-no-meta-v1");
    res.setHeader("Cache-Control", "s-maxage=900, stale-while-revalidate=86400");
    return res.status(200).json({ total, limit, offset, items });
  } catch (err) {
    console.error("[/api/products2] error:", err?.message, err?.stack);
