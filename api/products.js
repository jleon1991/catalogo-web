...
export default async function handler(req, res) {
  try {
    ...
    if (categ) domain.push(["categ_id", "ilike", categ]); // si diera problemas, lo cambiamos a búsqueda por IDs
    ...
    // ⬇️ importante: sin "__last_update", usamos "write_date"
    ...
    const products = recs.map(r => {
      const unique = encodeURIComponent(r.write_date || Date.now().toString());
      const imgUrl =
